...
public class mushroom_ablitity_control : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(timer>cool_down&&GetComponent<player_info>().Player_Direction==-1&&Input.GetKeyDown("e"))launch_rocket();
        if(timer>cool_down&&GetComponent<player_info>().Player_Direction==1&&Input.GetKeyDown("u"))launch_rocket();
    }
